...
export default function OutputPage() {
  ...
  const fetchOutput = useCallback(async () => {
    try {
      const result = await callApi<OutputData>('dashGetOutput');
      setData(result);
      setError(null);
    } catch (err: any) {
  ...
  useAutoRefresh(fetchOutput, { intervalMs: 5000 });
  ...
  if (!data?.content) {
    return (
      <div style={centered}>
        <div style={{ textAlign: 'center' }}>
          <p style={{ ...font, color: Colors.textMuted, fontSize: '1rem' }}>No output available</p>
          <button onClick={handleRefresh} style={{ ...font, background: Colors.surfaceLight, color: Colors.text, border: `1px solid ${Colors.border}`, borderRadius: 6, padding: '0.4rem 1rem', cursor: 'pointer' }}>
            Refresh
          </button>
        </div>
