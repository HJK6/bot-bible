"""
Shared dataclass models for the telegram-claude-bot project.

All data models should extend DataclassBase for consistent from_dict/to_dict support.
"""
...
class ScraperAction(DataclassBase):
    ...
    index: Optional[int] = None  # used by shadow DOM actions (type_shadow, click_shadow_button, keyboard_type)
...
@dataclass
class ScreenRegion(DataclassBase):
    """A rectangular region on screen, pixel coordinates."""
...
@dataclass
class BBox(DataclassBase):
    """Bounding box from vision model. Pixel coordinates."""
...
@dataclass
class LocateResult(DataclassBase):
...
@dataclass
class RPAResult(DataclassBase):
    """Result of an RPA action."""
